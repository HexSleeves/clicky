...
struct BlueCursorView: View {
    ...
    private let fullWelcomeMessage = "hey! i'm milo"
    ...
    var body: some View {
        ZStack {
                ...
                Text(welcomeText)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 6, style: .continuous)
                            ...
                            .shadow(color: companionManager.selectedCursorColor.glowColor.opacity(0.5), radius: 6, x: 0, y: 0)
                    )
                    .fixedSize()
                    .overlay(
                ...
                Text(navigationBubbleText)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 6, style: .continuous)
                            .fill(companionManager.selectedCursorColor.displayColor)
                            .shadow(
                                color: companionManager.selectedCursorColor.glowColor.opacity(0.5 + (1.0 - navigationBubbleScale) * 1.0),
            ...
            Triangle()
                ...
                .shadow(color: companionManager.selectedCursorColor.glowColor, radius: 8 + (buddyFlightScale - 1.0) * 20, x: 0, y: 0)
            ...
            // Cursor-color waveform — replaces the triangle while listening
            BlueCursorWaveformView(
                audioPowerLevel: companionManager.currentAudioPowerLevel,
                cursorColor: companionManager.selectedCursorColor.displayColor,
                cursorGlowColor: companionManager.selectedCursorColor.glowColor
            )
            ...
            // Cursor-color spinner — shown while the AI is processing (transcription + Claude + waiting for TTS)
...
private struct BlueCursorWaveformView: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 36.0)) { timelineContext in
            ...
            .shadow(color: cursorGlowColor.opacity(0.6), radius: 6, x: 0, y: 0)
